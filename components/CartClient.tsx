...
import { getCartPageData, removeCartItem, type CartLine, type CartPageData } from '@/lib/cart-actions';
...
/**
 * 카트 페이지가 예전엔 이 컴포넌트 자체가 라우트(page.tsx)였고 'use client'라서, 서버가 빈 껍데기
 * HTML만 먼저 보내고 그다음 JS 번들이 다운로드·파싱·hydration까지 끝난 뒤에야 비로소
 * getCartPageData()를 클라이언트에서 호출했다 — 다른 페이지(/looks 등)는 서버 컴포넌트가 렌더링
 * "중에" 데이터를 이미 들고 있는 HTML을 바로 보내는데, 카트만 그 왕복이 한 번 더 얹혀서
 * 체감 로딩이 눈에 띄게 느렸다. 지금은 app/(member)/cart/page.tsx(서버 컴포넌트)가 최초 데이터를
 * 미리 fetch해서 initialData로 넘겨주고, 이 컴포넌트는 그 값으로 상태를 바로 채운다 — 최초 로드에는
 * 클라이언트 쪽 데이터 요청이 아예 없다(장바구니 비우기 등 이후의 새로고침에서만 refresh()를 씀).
 */
export default function CartClient({ initialData }: { initialData: CartPageData }) {
  // 매 렌더마다 새로 계산 — 모듈 스코프 상수로 두면 자정을 넘겨 탭을 오래 열어둔 세션에서
  // "오늘"이 어제 날짜로 고정돼 지난 날짜가 예약 가능하게 보이는 버그가 생김.
  const TODAY = toDate(todayISO());
  const router = useRouter();
  const [items, setItems] = useState<CartLine[]>(initialData.items);
  const [busy, setBusy] = useState<Set<string>>(new Set(initialData.busyDates));
  const [closed, setClosed] = useState<Set<string>>(new Set(initialData.closedDates));
  const [month, setMonth] = useState(() => new Date(Date.UTC(TODAY.getUTCFullYear(), TODAY.getUTCMonth(), 1)));
  const [start, setStart] = useState<Date | null>(null);
  const [end, setEnd] = useState<Date | null>(null);
  const [slot, setSlot] = useState<string | null>(null);
  const [deliveryMethod, setDeliveryMethod] = useState<string | null>(initialData.profile?.preferredDeliveryMethod ?? null);
  const [pending, startTransition] = useTransition();
  const [err, setErr] = useState<string | null>(null);
  const [otherCartConflicts, setOtherCartConflicts] = useState<Set<string>>(initialData.otherConflicts);
  const [profile, setProfile] = useState<Profile | null>(initialData.profile);
  const [pickupPhone, setPickupPhone] = useState(initialData.profile?.deliveryPhone || initialData.profile?.phone || '');
    ...
    return (
      <section className="cart-page cart-wide">
        <div className="empty" style={{ textAlign: 'center', padding: '40px 0' }}>
          ...
          <Link href="/looks" className="cta ghost" style={{ marginTop: 14 }}>룩북 둘러보기</Link>
        </div>
      </section>
    );
  }
