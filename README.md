...
1. **스키마/시드** — SQL Editor에서 순서대로 실행
   `db/schema.sql` → `db/seed.sql` → `db/auth.sql` → `db/cart.sql` → `db/payments.sql` → `db/roles.sql` → `db/profile-fields.sql` → `db/username-auth.sql` → `db/fitting-delivery.sql` → `db/address-detail.sql` → `db/jibun-address.sql` → `db/entrance-password.sql` → `db/membership-fee.sql` → `db/push-subscriptions.sql` → `db/phone-verify-limit.sql` → `db/marketing-broadcast.sql` → `db/marketing-compliance.sql` → `db/marketing-schedule.sql` → `db/marketing-granular-consent.sql` → `db/address-change-log.sql` → `db/reservation-order-link.sql` → `db/fulfillment-status-v2.sql`
