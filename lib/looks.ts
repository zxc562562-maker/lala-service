...
function mapLook(r: LookRow): Look {
  ...
  return { id: r.id, coverUrl: getLookImageUrl(r.cover_path), images, items };
}

const LOOK_SELECT = 'id,cover_path,look_image(path,position),look_item(position,product:product_id(name))';
